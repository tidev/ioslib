		...
		async.parallel(
			{
				security: function (next) {
					appc.subprocess.findExecutable(
						[options.security, '/usr/bin/security', 'security'],
						function (err, result) {
							if (err) {
								results.issues.push({
									...
									type: 'error',
									message:
										__("Unable to find the 'security' executable.") +
										'\n' +
				...
				xcodeSelect: function (next) {
					appc.subprocess.findExecutable(
						[options.xcodeSelect, '/usr/bin/xcode-select', 'xcode-select'],
						function (err, result) {
							if (err) {
								results.issues.push({
									...
									type: 'error',
									message:
										__("Unable to find the 'xcode-select' executable.") +
										'\n' +
										__(
