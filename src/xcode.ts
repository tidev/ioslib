...
import { simulatorDevicePairCompatibility } from './sim-device-pairs.ts';
...
export async function detect(options: {
	...
	minTVosVersion?: string;
	minIosVersion?: string;
	minWatchosVersion?: string;
	sqlite?: string;
			...
			[
				...
				function detectOSXenv(next) {
					env.detect(options, function (err, env) {
						(Array.isArray(options.searchPath) ? options.searchPath : [options.searchPath]).forEach(
							function (p) {
								p && (searchPaths[p] = 1);
							}
						);
				...
				function findXcodes(next) {
					...
					Object.keys(searchPaths).forEach(function (p) {
						if (fs.existsSync(p) && fs.statSync(p).isDirectory()) {
							...
							if (
								/\/Contents\/Developer\/?$/.test(p) &&
								fs.existsSync(path.join(p, 'usr', 'bin', 'xcodebuild')) &&
								xcodes.indexOf(p) === -1
							) {
								xcodes.push(p);
							} else {
								...
								if (
									fs.existsSync(path.join(devDir, 'usr', 'bin', 'xcodebuild')) &&
									xcodes.indexOf(devDir) === -1
								) {
									xcodes.push(devDir);
								} else {
									...
									fs.readdirSync(p).forEach(function (name) {
										...
										if (
											xcodes.indexOf(dir) === -1 &&
											fs.existsSync(path.join(dir, 'usr', 'bin', 'xcodebuild'))
										) {
											xcodes.push(dir);
										}
									});
						...
						function (dir, cb) {
							var p = new appc.plist(path.join(path.dirname(dir), 'version.plist')),
								...
								supported = options.supportedVersions
									? appc.version.satisfies(version, options.supportedVersions, true)
									: true,
							...
							if (appc.version.gte(version, '7.0')) {
								watchos = {
									sdks: findSDKs(
										path.join(dir, 'Platforms', 'WatchOS.platform', 'Developer', 'SDKs'),
										/^WatchOS(.+)\.sdk$/,
										options.minWatchosVersion
									),
									sims: [],
								};
							...
							var tvos = {
								sdks: findSDKs(
									path.join(dir, 'Platforms', 'AppleTVOS.platform', 'Developer', 'SDKs'),
									/^AppleTVOS(.+)\.sdk$/,
									options.minTVosVersion
								),
								sims: [], // nobody cares
							};
							...
							var xc = (results.xcode[id] = {
								...
								eulaAccepted: false,
								sdks: findSDKs(
									path.join(dir, 'Platforms', 'iPhoneOS.platform', 'Developer', 'SDKs'),
									/^iPhoneOS(.+)\.sdk$/,
								...
								teams: {},
								executables: {
									xcodebuild: fs.existsSync((f = path.join(dir, 'usr', 'bin', 'xcodebuild')))
										? f
										: null,
									clang: fs.existsSync(
										(f = path.join(
								...
								if (appc.version.satisfies(xc.version, xcodeRange)) {
									...
									Object.keys(globalSimRuntimes).forEach(function (runtime) {
										Object.keys(xc.simDevicePairs).forEach(function (iosRange) {
											if (
												/iOS/.test(runtime) &&
												appc.version.satisfies(globalSimRuntimes[runtime].version, iosRange)
											) {
												...
												Object.keys(xc.simDevicePairs[iosRange]).forEach(function (watchosRange) {
													if (
														appc.version.satisfies(globalSimRuntimes[runtime].version, watchosRange)
													) {
														xc.simRuntimes[runtime] = globalSimRuntimes[runtime];
													}
												});
							...
							simctl.list({ simctl: xc.executables.simctl }, function (err, info) {
								...
								if (supported === false) {
									results.issues.push({
										...
										type: 'warning',
										message:
											__('Xcode %s is too old and is no longer supported.', '__' + version + '__') +
											'\n' +
											__(
												'The minimum supported Xcode version is Xcode %s.',
												appc.version.parseMin(options.supportedVersions)
											),
										xcodeVer: version,
									...
									results.issues.push({
										...
										type: 'warning',
										message:
											__('Xcode %s may or may not work as expected.', '__' + version + '__') +
											'\n' +
											__(
												'The maximum supported Xcode version is Xcode %s.',
												appc.version.parseMax(options.supportedVersions, true)
											),
										xcodeVer: version,
				...
				function findTeams(next) {
					appc.subprocess.findExecutable(
						[options.sqlite, '/usr/bin/sqlite3', '/usr/bin/sqlite', 'sqlite3', 'sqlite'],
						function (err, sqlite) {
							if (err) {
								results.issues.push({
							...
							async.each(
								Object.keys(results.xcode),
								function (id, cb) {
									var xc = results.xcode[id],
										dbFile = appc.fs.resolvePath(
											'~/Library/Developer/Xcode/DeveloperPortal ' + xc.version + '.db'
										);
									...
									appc.subprocess.run(
										...
										[
											...
											'-separator',
											'|||',
											'SELECT ZNAME, ZSTATUS, ZTEAMID, ZTYPE FROM ZTEAM',
										],
										function (code, out, err) {
											if (!code) {
												out
				...
				if (Object.keys(results.xcode).length) {
					...
					if (eulaNotAccepted.length) {
						...
						if (xcodeIds.length === 1) {
							message =
								__('Xcode EULA has not been accepted.') +
								'\n' +
								__('Launch Xcode and accept the license.');
						} else {
							message =
								__('Multiple Xcode versions have not had their EULA accepted:') +
								'\n' +
								eulaNotAccepted
									.map(function (xc) {
					...
					if (!sdkCounter) {
						results.issues.push({
							...
							type: 'error',
							message:
								__('There are no iOS SDKs found') +
								'\n' +
					...
					if (!sdkCounter) {
						results.issues.push({
							...
							type: 'error',
							message:
								__('There are no iOS Simulators found') +
								'\n' +
					...
					results.issues.push({
						...
						type: 'error',
						message:
							__('No Xcode installations found.') +
							'\n' +
							__(
								'You can download it from the %s or from %s.',
								'__App Store__',
