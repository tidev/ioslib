 ...
 * @param {Number} [options.timeout=180] - Number of seconds to wait for the simulator to launch and launch the app before timing out. Defaults to 180. Maximum of 500 seconds. Minimum of 90 seconds. Ignored if not installing an app.
			...
			detect(options, function (err, simInfo) {
				...
				function launchSim() {
					var existingCrashes = getCrashes(),
						timeout = Math.min(500, Math.max(options.timeout || 180, 90)), // default of 180 seconds, maximum of 500 seconds, minimum of 90 seconds
					...
					simProcess.stderr.on('data', function (data) {
						...
						if (output.indexOf('Simulator started (with session)') !== -1) {
