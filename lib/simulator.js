		...
		xcode.detect(options, function (err, xcodeInfo) {
			...
			detect(options, function (err, simulators) {
				...
				function startSimulator(simHandle, onStart, onStop) {
					...
					async.series([
						function (next) {
							isRunning(simHandle.simulator, function (err, running) {
								if (err) {
									return next(err);
								}
						...
						if (simHandle.systemLog) {
							...
							var systemLogRegExp = new RegExp(' ' + appName + '\\[(\\d+)\\]: (.*)'),
								...
								crash2RegExp = new RegExp(' (SpringBoard|Carousel)\\[(\\d+)\\]: Application \'.*\:' + appId + '\\[(\\w+)\\]\' crashed'),
				...
				async.series([
					...
					function (next) {
						...
						if (options.focus !== false && !options.hide && !options.autoExit) {
								...
								function (cb) {
									appc.subprocess.run('osascript', args, function (code, out, err) {
										if (!code) {
											focused = true;
											cb();
										} else if (/Application isn.t running/.test(err)) {
							...
							appc.subprocess.run('osascript', [ path.join(__dirname, 'iphone_sim_hide.scpt'), simHandle.simulator ], next);
						}
					},
					...
					function (next) {
						...
						if (watchSimHandle && (options.hide || options.autoExit)) {
							...
							appc.subprocess.run('osascript', [ path.join(__dirname, 'iphone_sim_hide.scpt'), watchSimHandle.simulator ], next);
						} else {
							next();
						}
					}
