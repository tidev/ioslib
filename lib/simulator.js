 ...
 * @param {Boolean} [options.watchApp] - The id of the watch app. Required in order to find a watch simulator.
	...
	xcode.detect(options, function (err, xcodeInfo) {
		...
		detect(options, function (err, simInfo) {
			...
			if (options.simHandleOrUDID) {
				...
				if (options.watchApp && !options.watchHandleOrUDID && !watchSimHandle) {
				...
				if (watchSimHandle) {
					if (options.watchApp && options.watchHandleOrUDID) {
						...
						if (!options.simVersion || simVers[j] === options.simVersion) {
							...
							for (var k = 0; !simHandle && k < sims.length; k++) {
								if (!options.simType || sims[k].family === options.simType) {
									if (!options.appPath || !options.watchApp) {
				...
				} else if (options.watchApp && !watchSimHandle) {
