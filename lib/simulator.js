							...
							JSON.parse(out).forEach(function (sim) {
								...
								if (!results.simulators[sim.version].some(function (s) { return s.udid === sim.udid; })) {
									var info = {
										...
										'logFile': null,
										'developerDir': xc.path,
										'args': []
									};
									
									// Define launch actions
									Object.defineProperty(info, 'launch',{
										'value': function () {
											emitter.emit('log-debug', 'Lauching xCode 6 simulator');
											simctlInstance = simctl.launch(this.udid,{
												appPath:this.appPath,
												developerDir: this.developerDir,
												timeout: this.timeout
											});
											return simctlInstance;
										}
									});

									results.simulators[sim.version].push(info);
								}
					...
					legacySims.forEach(function (sim) {
						if (sim.versions.indexOf(simVer) !== -1) {
							...
							if (!results.simulators[simVer].some(function (s) { return s.udid === udid; })) {
								...
								if (sim['64bit']) {
									info.args.push('--sim-64bit');
								}
									
								// Define launch actions
								Object.defineProperty(info, 'launch',{
									'value': function () {
										var simEmmiter =  new EventEmitter;

										var simProcess = spawn(iossim, ['launch', this.appPath, '--verbose', '--xcode-dir', this.developerDir, '--sdk', this.sdk, '--family', this.type].concat(this.args));
										simProcess.stdout.on('data', function (data) {
											simEmmiter.emit('log', data.toString());
										});

										simProcess.stderr.on('data', function (data) {
											simEmmiter.emit('error', data.toString());
										});

										simProcess.on('close', function (code, signal) {
											simEmmiter.emit('close', {code:code, signal:signal});
										});
										return simEmmiter;
									}
								});
...
function launch(udid, options, callback) {
	...
	xcode.detect(options, function (err, xcodeInfo) {
		...
		detect(options, function (err, simInfo) {
			...
			function launchSim() {
				simHandle.timeout = Math.max(options.timeout || 180, 1); // default of 180 seconds, minimum of 1 seconds
				simHandle.appPath = options.appPath;

				var existingCrashes = getCrashes(),
					simProcess = simHandle.launch(),
					appName = options.appName || (options.appPath ? path.basename(options.appPath) : null),
					findLogTimer = null,
					tail;

				simProcess.on('error', function (value) {
				...
				if (appName) {
					var autoExitToken = options.autoExitToken || 'AUTO_EXIT',
						logRegExp = new RegExp(appName.replace(/\.app$/, '') + '\\[[^\\]]*]:(.*)$');

					// listen for NSLog() output
					simProcess.on('log', function (value) {
						var exit = false,
							output = value.toString();
						output.split('\n').forEach(function (line) {
							emitter.emit('log-debug', '[ios-sim] ' + line.trim());
						});
						output.split('\n').forEach(function (line) {
							emitter.emit('log-raw', line.trim());

							var m = line.match(logRegExp);
							m && emitter.emit('log', m[1].trim());
				...
				simProcess.on('close', function (value) {
					var code = value.code;
