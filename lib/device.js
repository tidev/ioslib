...
function runDevicectl(args, callback) {
	// note: '-' streams the JSON to stdout; '/dev/stdout' would break subcommands
	// like 'device info' that save their JSON output atomically
	execFile('xcrun', [ 'devicectl', ...args, '--quiet', '--json-output', '-' ], EXEC_LIMIT, function (err, stdout, stderr) {
		var result = null,
			parseErr = null;
		try {
