...
/**
 * stop a pending install by sending a HUP to the process
 */
function stop(next) {
	exec("/bin/ps -ef | /usr/bin/egrep '(lldb|ios-deploy)' | /usr/bin/grep -v grep | /usr/bin/awk '{print $2}' | /usr/bin/sort | /usr/bin/head -n 1 | /usr/bin/xargs kill -HUP", next || function(){});
}
...
function _launch (obj) {
	...
	function run () {
		var i = path.join(ios_deploy,'ios-deploy'),
			args = ['--debug', '--verbose',(uninstall ? '--uninstall':''), '--unbuffered','--noninteractive', '--bundle', build_dir, '--id', device_id, '--timeout', 10],
