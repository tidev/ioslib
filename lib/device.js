	...
	return magik(options, null, function (handle, options) {
		...
		function attempt() {
			...
			child.on('exit', function (code) {
				...
				if (remainder.trim()) {
					sawOutput = true;
					handle.emit('log', remainder);
					remainder = '';
				}
				if (code && !sawOutput) {
					// launch failed before any output, check if the device is simply locked
					return lockState(udid, function (err, locked) {
						if (locked) {
							waitForUnlock();
						} else {
							handle.emit('error', new Error(__('Failed to launch app on device')));
						}
					});
				}
			...
			(function poll() {
				...
				lockState(udid, function (err, locked) {
					if (locked === true) {
						...
						// unlocked, or state unknown: just try the launch
						attempt();
					}
				});
			}());
