...
function build(app, iosVersion, defs, done) {
	...
	ioslib.xcode.detect(function (err, env) {
		...
		if (!xc.eulaAccepted) {
			return done(
				new Error(
					'Xcode must be launched and the EULA must be accepted before the iOS app can be compiled.'
				)
			);
		}
		...
		var args = [
			...
			"platform='iOS Simulator',OS=" +
				appc.version.format(iosVersion, 2, 2) +
...
describe('simulator', function () {
	...
	(process.env.CI ? it.skip : it)(
		'iOS Sim + bad Watch Sim UDID + no watch app is valid',
		function (done) {
			this.timeout(8000);
			this.slow(2000);
	...
	(process.env.CI ? it.skip : it)(
		'fail with good iOS Sim UDID + bad Watch Sim UDID + watch app',
		function (done) {
			this.timeout(8000);
			this.slow(2000);
	...
	(process.env.CI ? it.skip : it)(
		'no iOS Sim + Watch Sim + no watch app is valid',
		function (done) {
			this.timeout(8000);
			this.slow(2000);
	...
	(process.env.CI ? it.skip : it)(
		'no iOS Sim + no Watch Sim + no watch app is valid',
		function (done) {
			this.timeout(8000);
			this.slow(2000);
	...
	(process.env.CI ? it.skip : it)(
		'no iOS Sim + app + no Watch Sim + no watch app is valid',
		function (done) {
			this.timeout(8000);
			this.slow(2000);
	...
	(process.env.CI ? it.skip : it)(
		'no iOS Sim + app + no Watch Sim + watch app is valid',
		function (done) {
			this.timeout(8000);
			this.slow(2000);
	...
	(process.env.CI ? it.skip : it)(
		'no iOS Sim + app + Watch Sim + watch app is valid',
		function (done) {
			this.timeout(8000);
			this.slow(2000);
		...
		ioslib.simulator.findSimulators(
			...
			function (err, simHandle, watchSimHandle, selectedXcode, simInfo, xcodeInfo) {
				...
				should(err.message).equal(
					`Unable to find any Watch Simulators that can be paired with the specified iOS Simulator ${ipadSim}.`
				);
				done();
			}
		);
	...
	(process.env.CI ? it.skip : it)(
		'should launch the default simulator and stop it',
		function (done) {
			this.timeout(120000);
			this.slow(60000);

			ioslib.simulator
				.launch(iphoneSim, null, function (err, simHandle, watchSimHandle) {
					...
					appc.subprocess.run('ps', '-ef', function (code, out, err) {
						...
						should(
							out.split('\n').filter(function (line) {
								return line.indexOf(simHandle.simulator) !== -1;
							})
						).not.length(0);
	...
	(process.env.CI ? it.skip : it)(
		'should be able to launch simulator and log basic logs',
		function (done) {
			this.timeout(120000);
			this.slow(60000);
			...
			build('TestApp', null, ['TEST_BASIC_LOGGING'], function (err, appPath) {
				...
				ioslib.simulator
					...
					.on('log-debug', function (line, simHandle) {
						logger(
							(simHandle ? '[' + simHandle.family.toUpperCase() + '] ' : '') + '[DEBUG]',
							line
						);
					})
	...
	(process.env.CI ? it.skip : it)(
		'should be able to launch simulator and log ti mocha results',
		function (done) {
			this.timeout(60000);
			this.slow(60000);
	...
	(process.env.CI ? it.skip : it)(
		'should be able to launch simulator and log ti mocha results with multiple lines',
		function (done) {
			this.timeout(120000);
			this.slow(60000);
	...
	(process.env.CI ? it.skip : it)(
		'should be able to launch simulator and detect crash with Objective-C exception',
		function (done) {
			this.timeout(60000);
			this.slow(60000);
	...
	(process.env.CI ? it.skip : it)(
		'should launch the default simulator and launch the watchOS 2 app',
		function (done) {
			this.timeout(120000);
			this.slow(60000);
